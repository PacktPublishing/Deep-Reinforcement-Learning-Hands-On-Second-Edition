...
if __name__ == "__main__":
    ...
    epsilon_tracker_enabled = not params.eps_decay_trigger
    ...
    def process_batch(engine, batch):
        if not epsilon_tracker_enabled:
            return {
                "loss": 0.0,
    ...
    def check_good_reward(trainer: Engine):
        global epsilon_tracker_enabled, epsilon_tracker_frame
        if epsilon_tracker_enabled:
            return
        # check trigger condition to enable epsilon decay
        if trainer.state.episode_reward > -200:
            epsilon_tracker_enabled = True
